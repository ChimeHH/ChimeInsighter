...
class IastPipelineManager(PipelineManager):
    class Context(WorkerContext):
        def __init__(self, sastmsg):
            ...
            self.metadata = sastmsg.data.get('metadata', None)

    def __init__(self, engine, max_workers):
        super().__init__(engine, max_workers=max_workers)

class IastEngine(SlaverEngine):
    def __init__(self):
        super().__init__('iast', IastPipelineManager)
