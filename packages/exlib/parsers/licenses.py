...
class LicenseClassifier:
    ...
    def init_once(cls): 
        if not cls.__ROOT:       
            cls.__ROOT = os.path.dirname(__file__)

            # Shared Library Initialization
            cls.__so = ctypes.cdll.LoadLibrary(os.path.join(cls.__ROOT, "compiled/libmatch.so"))
            cls.__init = cls.__so.CreateClassifier
            cls.__init.argtypes = [ctypes.c_char_p, ctypes.c_double]

            cls.__scanfile = cls.__so.ScanFile
            cls.__scanfile.argtypes = [ctypes.c_char_p, ctypes.c_int, ctypes.c_bool]
            cls.__scanfile.restype = ctypes.c_char_p
...
def guess_licenses(filepath):
    license_classifier = LicenseClassifier()
    return license_classifier.scan_file(filepath)
