...
class Client(object):
    ...
    urls = {'overview': 'api/overview',
            ...
            'all_users': 'api/users',
            'all_permissions': 'api/permissions',
            'all_bindings': 'api/bindings',
            'name': 'api/name',
            'queues_by_vhost': 'api/queues/%s',
            'queues_by_name': 'api/queues/%s/%s',
            'exchanges_by_vhost': 'api/exchanges/%s',
            'exchange_by_name': 'api/exchanges/%s/%s',
            'live_test': 'api/aliveness-test/%s',
            'purge_queue': 'api/queues/%s/%s/contents',
            'channels_by_name': 'api/channels/%s',
            'connections_by_name': 'api/connections/%s',
            'bindings_by_source_exch': 'api/exchanges/%s/%s/bindings/source',
            'bindings_by_dest_exch': 'api/exchanges/%s/%s/bindings/destination',
            'bindings_on_queue': 'api/queues/%s/%s/bindings',
            'bindings_between_exch_queue': 'api/bindings/%s/e/%s/q/%s',
            'rt_bindings_between_exch_queue': 'api/bindings/%s/e/%s/q/%s/%s',
            'get_from_queue': 'api/queues/%s/%s/get',
            'publish_to_exchange': 'api/exchanges/%s/%s/publish',
            'vhosts_by_name': 'api/vhosts/%s',
            'vhost_permissions': 'api/permissions/%s/%s',
            'users_by_name': 'api/users/%s',
            'user_permissions': 'api/users/%s/permissions',
            'vhost_permissions_get': 'api/vhosts/%s/permissions'
            }
    ...
    def __init__(self, api_url, user, passwd, timeout=5, scheme='http'):
        ...
        # print(f"url: {api_url}, user: {user}, passwd: {passwd}, timeout: {timeout}, scheme: {scheme}")
    ...
    def get_messages(self, vhost, qname, count=1,
        ...
        :param string ackmode: ack_requeue_true/reject_requeue_true, Whether to requeue the message after getting it.
