import { severityMap, securityTypeMap } from '@/utils/constant';
...
const securityDetail = () => {
  ...
  const [detail, setDetail] = useState<SecurityDetail>();
    ...
    <>
      ...
      <Descriptions title={ formatMessage({ id: `${baseMessageId}.properties` }) } className={styles.panel} column={2}>
        ...
        <Descriptions.Item label={formatMessage({ id: `${baseMessageId}.type` })}>
          {detail?.sub_type && securityTypeMap[detail.sub_type]}
        </Descriptions.Item>        
      </Descriptions>

      <Descriptions
        title={formatMessage({ id: `${baseMessageId}.list` })}
          ...
          <table className={styles.table} style={{ borderCollapse: 'collapse', width: '100%' }}>
            <thead>
              <tr style={{ backgroundColor: '#f5f5f5' }}>
                <th style={{ width: '10%', textAlign: 'left', border: '1px solid #ddd' }}>{formatMessage({ id: `${baseMessageId}.num_index` })}</th>
                <th style={{ width: '90%', textAlign: 'left', border: '1px solid #ddd' }}>{formatMessage({ id: `${baseMessageId}.data` })}</th>
              </tr>
            </thead>
            <tbody>
              {Array.isArray(detail?.metadata?.data) ? (
                detail.metadata.data.map((item, index) => (
                  <tr key={index}>
                    <td className={styles.td} style={{ width: '10%', textAlign: 'left', border: '1px solid #ddd' }}>{index + 1}</td>
                    <td className={styles.td} style={{ width: '90%', textAlign: 'left', border: '1px solid #ddd' }}>{item}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td className={styles.td} style={{ width: '10%', textAlign: 'left', border: '1px solid #ddd' }}>1</td>
                  <td className={styles.td} style={{ width: '90%', textAlign: 'left', border: '1px solid #ddd' }}>{JSON.stringify(detail?.metadata)}</td>
                </tr>
              )}
            </tbody>
          </table>
