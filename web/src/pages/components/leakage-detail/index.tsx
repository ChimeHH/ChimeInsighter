import { severityMap, infoleakTypeMap } from '@/utils/constant';
...
const infoleakDetail = () => {
  const { vid, tid } = useParams<{ vid: string; tid: string }>();
  const [version, setVersion] = useState<Version>();
  const [detail, setDetail] = useState<InfoleakDetail>();
  const [treeData, setTreeData] = useState<DataNode[]>([]);
  const { formatMessage } = useIntl();

  const initPaths = (threatDetail: InfoleakDetail) => {
  ...
  return (
    <>
      ...
      <Descriptions title={ formatMessage({ id: `${baseMessageId}.properties` }) } className={styles.panel} column={2}>        
        ...
        <Descriptions.Item label={formatMessage({ id: `${baseMessageId}.length` })}>
          {detail?.metadata.length}
        </Descriptions.Item>         
        
        {detail?.metadata?.pattern && (
          <Descriptions.Item label={formatMessage({ id: `${baseMessageId}.rule` })}>
            {detail?.metadata.pattern}
          </Descriptions.Item>
        )}

        {/* {detail?.metadata?.reference && (
      ...
      <Descriptions
        title={formatMessage({ id: `${baseMessageId}.reference` })}
        className={styles.panel}
      >
        {detail?.metadata?.reference && Object.entries(detail.metadata.reference).map(([key, value]) => (
