...
const MisconfigurationDetail = () => {
  const { vid, tid } = useParams<{ vid: string; tid: string }>();
  const [version, setVersion] = useState<Version>();
  const [detail, setDetail] = useState<MisconfigurationDetail>();
  const [treeData, setTreeData] = useState<DataNode[]>([]);
  const { formatMessage } = useIntl();

  const initPaths = (threatDetail: MisconfigurationDetail) => {
...
export default MisconfigurationDetail;
